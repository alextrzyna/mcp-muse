...
impl FunDSPSynth {
    ...
    pub fn generate_samples(&self, params: &FunDSPParams) -> Result<Vec<f32>> {
        ...
        match &params.synth_type {
            FunDSPSynthType::Kick { punch, sustain, click_freq, body_freq } => {
                self.generate_kick_samples(*punch, *sustain, *click_freq, *body_freq, params.amplitude, sample_count)
            },
            FunDSPSynthType::Snare { snap, buzz, tone_freq, noise_amount } => {
                self.generate_snare_samples(*snap, *buzz, *tone_freq, *noise_amount, params.amplitude, sample_count)
            },
            FunDSPSynthType::HiHat { metallic, decay, brightness } => {
                self.generate_hihat_samples(*metallic, *decay, *brightness, params.frequency, params.amplitude, sample_count)
            },
            FunDSPSynthType::Cymbal { size, metallic, strike_intensity } => {
                self.generate_cymbal_samples(*size, *metallic, *strike_intensity, params.frequency, params.amplitude, sample_count)
            },
            FunDSPSynthType::FM { modulator_freq, modulation_index } => {
                self.generate_fm_samples(params.frequency, *modulator_freq, *modulation_index, params.amplitude, sample_count)
            },
            FunDSPSynthType::Granular { grain_size, overlap, density } => {
                self.generate_granular_samples(params.frequency, *grain_size, *overlap, *density, params.amplitude, sample_count)
            },
            FunDSPSynthType::Zap { energy, decay, harmonic_content } => {
                self.generate_zap_samples(params.frequency, *energy, *decay, *harmonic_content, params.amplitude, sample_count)
            },
            FunDSPSynthType::Swoosh { direction, intensity, frequency_sweep } => {
                self.generate_swoosh_samples(*direction, *intensity, *frequency_sweep, params.amplitude, sample_count)
            },
            FunDSPSynthType::Pad { warmth, movement, space, harmonic_evolution } => {
                self.generate_pad_samples(params.frequency, *warmth, *movement, *space, *harmonic_evolution, params.amplitude, sample_count)
            },
            FunDSPSynthType::Texture { roughness, evolution, spectral_tilt, modulation_depth } => {
                self.generate_texture_samples(params.frequency, *roughness, *evolution, *spectral_tilt, *modulation_depth, params.amplitude, sample_count)
            },
        }
    }

    /// Professional kick drum synthesis using FunDSP
    fn generate_kick_samples(&self, punch: f32, sustain: f32, click_freq: f32, body_freq: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        ...
        for i in 0..sample_count {
            ...
            let click_component = (2.0 * std::f32::consts::PI * click_freq * t).sin() * click_envelope * punch;
            ...
            let body_component = (2.0 * std::f32::consts::PI * body_freq_sweep * t).sin() * body_envelope;
    ...
    fn generate_snare_samples(&self, snap: f32, _buzz: f32, tone_freq: f32, noise_amount: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        let mut samples = Vec::with_capacity(sample_count);
        ...
        for i in 0..sample_count {
            ...
            let filtered_noise = if tone_freq > 1000.0 { noise } else { noise * 0.5 };
    ...
    fn generate_hihat_samples(&self, metallic: f32, decay: f32, brightness: f32, base_freq: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        let mut samples = Vec::with_capacity(sample_count);
        ...
        for i in 0..sample_count {
            ...
            let sample = (metallic1 + metallic2 + metallic3 + filtered_noise) * envelope_value * amplitude;
            samples.push(sample);
        }
    ...
    fn generate_cymbal_samples(&self, size: f32, metallic: f32, strike_intensity: f32, base_freq: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        // Use hihat with modified parameters for cymbal
        self.generate_hihat_samples(metallic, size, strike_intensity, base_freq, amplitude, sample_count)
    }

    /// FM synthesis for complex harmonic content
    fn generate_fm_samples(&self, carrier_freq: f32, modulator_freq: f32, modulation_index: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        let mut samples = Vec::with_capacity(sample_count);
    ...
    fn generate_granular_samples(&self, frequency: f32, _grain_size: f32, _overlap: f32, density: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        let mut samples = Vec::with_capacity(sample_count);
    ...
    fn generate_zap_samples(&self, frequency: f32, energy: f32, decay: f32, harmonic_content: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        let mut samples = Vec::with_capacity(sample_count);
        ...
        for i in 0..sample_count {
            ...
            let sample = (harmonic_sum * harmonic_content_factor + noise_component) * envelope_value * amplitude;
            samples.push(sample);
        }
    ...
    /// Swoosh sound effect
    fn generate_swoosh_samples(&self, direction: f32, intensity: f32, frequency_sweep: (f32, f32), amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        let mut samples = Vec::with_capacity(sample_count);
    ...
    fn generate_pad_samples(&self, frequency: f32, warmth: f32, _movement: f32, space: f32, _harmonic_evolution: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        let mut samples = Vec::with_capacity(sample_count);
        ...
        for i in 0..sample_count {
            ...
            let mut sample = (fundamental + harmonic2 + harmonic3 + harmonic4) * envelope_value * amplitude;
    ...
    fn generate_texture_samples(&self, frequency: f32, roughness: f32, _evolution: f32, _spectral_tilt: f32, _modulation_depth: f32, amplitude: f32, sample_count: usize) -> Result<Vec<f32>> {
        let mut samples = Vec::with_capacity(sample_count);
        ...
        for i in 0..sample_count {
            ...
            let sine_component = (2.0 * std::f32::consts::PI * frequency * t).sin() * (1.0 - roughness);
...
impl From<crate::expressive::synth::SynthParams> for FunDSPParams {
    fn from(params: crate::expressive::synth::SynthParams) -> Self {
        let synth_type = match params.synth_type {
            crate::expressive::synth::SynthType::Kick { punch, sustain, click_freq, body_freq } => {
            ...
            crate::expressive::synth::SynthType::Snare { snap, buzz, tone_freq, noise_amount } => {
            ...
            crate::expressive::synth::SynthType::HiHat { metallic, decay, brightness } => {
            ...
            crate::expressive::synth::SynthType::Cymbal { size, metallic, strike_intensity } => {
            ...
            crate::expressive::synth::SynthType::FM { modulator_freq, modulation_index } => {
                FunDSPSynthType::FM {
                    modulator_freq,
                    modulation_index,
                }
            },
            crate::expressive::synth::SynthType::Granular { grain_size, overlap, density } => {
            ...
            crate::expressive::synth::SynthType::Zap { energy, decay, harmonic_content } => {
                FunDSPSynthType::Zap {
                    energy,
                    decay,
                    harmonic_content,
            ...
            crate::expressive::synth::SynthType::Swoosh { direction, intensity, frequency_sweep } => {
            ...
            crate::expressive::synth::SynthType::Pad { warmth, movement, space, harmonic_evolution } => {
                FunDSPSynthType::Pad {
                    warmth,
                    movement,
                    space,
            ...
            crate::expressive::synth::SynthType::Texture { roughness, evolution, spectral_tilt, modulation_depth } => {
