...
impl R2D2Voice {
    ...
    pub fn generate_expression_params(
        &self,
        expression: &R2D2Expression,
    ) -> Option<R2D2SynthParams> {
        ...
        let duration = expression.duration
            * emotion_params.duration_multiplier
    ...
    pub fn create_phrase(&self, expression: &R2D2Expression) -> Vec<R2D2SynthParams> {
        ...
        for i in 0..syllable_count {
            ...
            syllable_expression.intensity =
                (expression.intensity + intensity_variation).clamp(0.0, 1.0);
